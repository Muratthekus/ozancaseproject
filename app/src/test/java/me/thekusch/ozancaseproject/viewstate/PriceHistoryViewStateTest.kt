...
class PriceHistoryViewStateTest {

    @Test
    fun `should return loading true when status is loading`() {
        // Given
        val givenViewState = PriceHistoryViewState(status = Status.LOADING)
    ...
    fun `should not return loading false when status is error`() {
        // Given
        val givenViewState = PriceHistoryViewState(status = Status.SUCCESS)
    ...
    fun `should not return loading false when status is success`() {
        // Given
        val givenViewState = PriceHistoryViewState(status = Status.ERROR)
